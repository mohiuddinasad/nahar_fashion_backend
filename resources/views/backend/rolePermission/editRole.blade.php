...
<style>
    ...
    .back-btn { display: inline-flex; align-items: center; justify-content: center; width: 36px; height: 36px; border-radius: 8px; background: white; border: 1px solid var(--border); color: var(--gray); text-decoration: none; font-size: 14px; transition: 0.2s; }
    ...
    .card-header { padding: 18px 24px; border-bottom: 1px solid var(--border); background: #f8f9ff; display: flex; align-items: center; justify-content: space-between; }
    .card-header h2 { font-size: 16px; font-weight: 700; color: var(--dark); margin: 0; }
    .core-badge { font-size: 11px; padding: 4px 10px; background: #fef9c3; color: #854d0e; border-radius: 20px; font-weight: 600; }
    .card-body { padding: 24px; }
    .form-group { margin-bottom: 22px; }
    .form-label { display: block; font-size: 13px; font-weight: 600; color: var(--dark); margin-bottom: 6px; }
    .form-label span { color: var(--danger); }
    .form-control { width: 100%; padding: 10px 14px; border: 1px solid var(--border); border-radius: 8px; font-size: 14px; color: #374151; background: white; box-sizing: border-box; transition: 0.2s; }
    .form-control:focus { outline: none; border-color: var(--primary); box-shadow: 0 0 0 3px rgba(79,70,229,0.1); }
    .form-control:disabled { background: #f3f4f6; color: var(--gray); cursor: not-allowed; }
...
<div class="rp-wrapper">
    ...
    <div class="page-header">
        ...
        <div>
            <h1 class="page-title">Edit Role: {{ ucfirst(str_replace('_', ' ', $role->name)) }}</h1>
            <p class="page-subtitle">Update permissions for this role</p>
        </div>
    </div>
    ...
    <div class="card">
        <div class="card-header">
            ...
            @if(in_array($role->name, ['super_admin','admin','manager','customer','wholesale_buyer']))
                <span class="core-badge">⚠️ Core Role</span>
            @endif
        </div>
        <div class="card-body">
            <form action="{{ route('dashboard.role-permission.role-update', $role->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="form-group">
                    ...
                    @if(in_array($role->name, ['super_admin','admin','manager','customer','wholesale_buyer']))
                        ...
                        <p class="form-hint"><i class="fas fa-lock"></i> Core role names cannot be changed.</p>
                    @else
                ...
                <div class="form-group">
                    <div class="perm-section-title">
                        <span><i class="fas fa-key" style="color:var(--primary);margin-right:6px;"></i> Permissions</span>
                    ...
                    <div class="perm-groups">
                        ...
                        @php $allChecked = $perms->every(fn($p) => in_array($p->name, $rolePermissions)); @endphp
